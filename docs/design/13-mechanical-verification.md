...
## Recommended design
1. **`--verify "CMD"`** (opt-in, explicit). Run `CMD` in cwd with a timeout. Non-zero exit ⇒ inject a synthetic `mechanically_verified` **critical** concern (`verify_command_failed`) with stdout/stderr tail as evidence; this hard-gates (`request_changes`). Clean exit ⇒ a recorded passing check, surfaced in the manifest.
