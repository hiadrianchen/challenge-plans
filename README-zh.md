...
- 📋 **任何计划**（`--type plan`）—— 旅行、上线、招聘、搬家都能审。比如一份京都行程：Day2 塞了六个点、还现在就订了不可退机票——它会告诉你哪天根本跑不完、哪步回不了头。
- 📝 **一份成型的 spec / 设计稿**（`--type spec`）—— 动手前先过一遍。比如「异步导出用户 CSV」：怎样算导出成功？谁来验收？漏了哪条边界？省得你照着含糊的 spec 做出错的东西。
- 🔧 **一次代码改动**（`--type diff`）—— 轻量 review。把 `git diff` 丢进去，它盯着你改了函数却没跟着改的调用点、没补的测试、悄悄破掉的兼容性。
- 🧭 **一个你已经做出的决定**（`--type decision`）—— 技术选型、选供应商、招人都行。比如「我们决定把项目重写成 TS」：更便宜的替代方案（先把环境清干净）你公平比过吗？理由是不是沉没成本？这步回得了头吗？

**选**（`weigh`）是另一种模式：在几个**还没定**的选项间拿不准时用——技术上的选型也好，生活里的选择也好。比如在 Postgres / MongoDB / SQLite 之间选、或在三个 offer 里挑一个——它让多个 AI 投票排序，但给每个模型家族的票封顶（免得一个聒噪的模型制造假共识），并把最强的反对意见摆上台面：给你一个带「为什么」和「谁不同意」的排序，而不是一句武断答案。
